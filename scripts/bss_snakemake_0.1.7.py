...
# printed output interferes with dag output
# should perhaps be called "not quiet" instead?
verbose = False
...
rmd_path = "/scripts"
rmd_file = "bss_summary_0.1.7.Rmd"
r_file = "bss_summarize_run_0.1.7.R"
...
if not os.path.isdir(bam_dir) :
    bam_dir = "BAM"
    if not os.path.isdir(bam_dir) :
        print("no directory called UBAM or BAM found")
        quit()
...
if verbose: print(BAMFILES)

## allow override?: info_file = config["info_file"]
# skip / if proj_dir = ""
info_file = glob.glob("*sample*.csv")
...
if len(info_file) > 0 :
    ...
    if "Sample Name (required)" in samp_info.columns :
        ...
        if verbose: print("'Sample Name (required)' and 'Sample Name' not found in samp_info, using", names_col)

    BAMDICT = {}
    for i in BAMFILES:
        ## TODO: match arbitrary strings from samp_info as prefix?
        BAMSTEM = re.search("(IonXpress_[0-9]+)_",i).group(1)
        if BAMSTEM in samp_info.index:
            BAMDICT[i] = samp_info[name_col][BAMSTEM]
        else:
            if verbose: print("no match for ", BAMSTEM, " in sample_info")
            BAMDICT[i] = BAMSTEM
        if verbose: print(i, ' -> ', BAMSTEM, '->', BAMDICT[i])
...
rule downsample_bam:
    ...
    shell:
        "python /scripts/bss_downsample_bam_0.1.7.py {params} {max_per_chrom} 2> {log.err} 1> {log.out}"
...
rule downsample_bam_100:
    ...
    shell:
        "python /scripts/bss_downsample_bam_0.1.7.py {params} 100 100 2> {log.err} 1> {log.out}"
...
rule bismark_hs:
    input:
        ...
        temp=bismark_temp
    output:
        bam = "bismark_bam/local.{sample}_bismark_hisat2.bam",
        ser = "bismark_bam/local.{sample}_bismark_hisat2_SE_report.txt"
    log:
