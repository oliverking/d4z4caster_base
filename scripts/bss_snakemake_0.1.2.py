...
rmd_path = "/scripts"
rmd_file = "bss_summary_0.1.2.Rmd"
...
rule all:
    input:
        expand(f"bismark_bam/local.{{id}}_bismark_{mapper}.sorted.bam.bai", id=INVDICT.keys()),
...
rule bismark_bt:
    input:
        fq="fastq/{sample}.fastq.gz",
        ix=ref_folder,
        ix2=f"{ref_folder}/Bisulfite_Genome/GA_conversion/BS_GA.1.bt2",
        temp=bismark_temp
    output:
...
rule bismark_me:
    input:
        "bismark_bam/local.{sample}_bismark_{mapper}.bam"
    output:
        "bismark_txt/any_C_context_local.{sample}_bismark_{mapper}.txt.gz"
    log:
        err = "logs/bismark_me_{mapper}/{sample}.err",
        out = "logs/bismark_me_{mapper}/{sample}.out"
    shell:
...
rule bismark_me2:
    input:
        "bismark_bam/local.{sample}_bismark_{mapper}.bam"
    output:
        "bismark_txt/CpG_OT_local.{sample}_bismark_{mapper}.txt.gz" ## etc
    log:
        err = "logs/bismark_me2_{mapper}/{sample}.err",
        out = "logs/bismark_me2_{mapper}/{sample}.out"
    shell:
        "{bismark_path}/bismark_methylation_extractor --output bismark_txt -gz "
...
rule cpg_matrix:
    input:
        script=rmd_local,
        txt=f"bismark_txt/any_C_context_local.{{sample}}_bismark_{mapper}.txt.gz"
    output:
    ...
    params:
        stub=f"r_out/report_{{sample}}"
    log:
        err=f"logs/r/{proj_name}{{sample}}.err",
        out=f"logs/r/{proj_name}{{sample}}.out"
    shell:
